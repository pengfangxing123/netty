...
public class FileWriteReadDemo {
    ...
    public byte[] readFile(String path) throws Exception {
        ...
        AsynchronousFileChannel ch = AsynchronousFileChannel.open(Paths.get(path), StandardOpenOption.READ);
        //读取文件临时ByteBuffer对象
     ...
     * @throws Exception
     */
    public void writeFile(String target ,byte[] bytes) throws Exception {
     ...
     * @throws IOException
     */
    public byte[] readFileFuture(String target) throws IOException {
        //打开文件通道,获取异步Channel对象
        AsynchronousFileChannel channel = AsynchronousFileChannel.open(Paths.get(target), StandardOpenOption.READ);
        //读取文件临时ByteBuffer对象
        ByteBuffer buffer = ByteBuffer.allocate(1024);
        //文件读取位置
        long position = 0;
     ...
     * @throws IOException
     */
    public void writeFileFuture(String target ,byte[] bytes) throws IOException {
        //若文件p.txt不存在,则创建该文件
        Path path = Paths.get(target);
        if (!Files.exists(path)) {
            Files.createFile(path);
